...
const AIConsultant: React.FC<AIConsultantProps> = ({ symbol, language }) => {
  ...
  useEffect(() => {
    const fetchMarketData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000'}/api/price/${symbol}`);
        if (response.ok) {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000'}/api/ai/analysis`, {
          method: 'POST',
  ...
  const handleSendMessage = async (question?: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000'}/api/ai/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
